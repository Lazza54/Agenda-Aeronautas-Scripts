...
TABELA_B1_PATH = pathlib.Path("R:/SPECTRUM_SYSTEM/Aeronautas/Documentos_Comuns/Arquivos_Diversos/Tabela B.1.json")
...
ARQUIVOS_DIVERSOS_PATH = pathlib.Path("R:/SPECTRUM_SYSTEM/Aeronautas/Documentos_Comuns/Arquivos_Diversos")
ARQUIVOS_DIVERSOS_FALLBACK_PATH = pathlib.Path("G:/SPECTRUM_SYSTEM/Aeronautas/Documentos_Comuns/Arquivos_Diversos")
FOLGAS_LATAM_PATH = ARQUIVOS_DIVERSOS_PATH / "folgas_regulamentares_LATAM.json"
FOLGAS_REGULAMENTARES_PATH = ARQUIVOS_DIVERSOS_PATH / "folgas_regulamentares.json"
FOLGAS_JSON_PATH = ARQUIVOS_DIVERSOS_PATH / "folgas.json"

# Atividades contadas como folga, mas fora da auditoria de conformidade (ex.: feriado operacional).
ACTIVITIES_EXCLUDED_FROM_CONFORMITY_AUDIT: Set[str] = {"FER"}
...
class LimitsLoader:
    ...
    Fontes:
      - Tabela B.1            → limites FDP/VOO para tripulação simples
    ...
    def __init__(self, json_dir: pathlib.Path = REGRAS_JSON_PATH):
        ...
        self._b1_rows: List[Dict[str, str]] = []  # Tabela B.1 rows loaded from JSON
        self._b1_source_available: bool = False
        ...
        self._special_duty_limit_ranges: Dict[str, Dict[str, Dict[str, Optional[int]]]] = {"reserva": {}}
    ...
    def _load(self, json_dir: pathlib.Path):
        ...
        if a3_path.exists():
            try:
                ...
                pass
        # --- Tabela B.1 ---
        if TABELA_B1_PATH.exists():
            try:
                with open(TABELA_B1_PATH, encoding='utf-8') as f:
                    b1_data = json.load(f)
                if isinstance(b1_data, list) and len(b1_data) >= 3:
                    self._b1_rows = [row for row in b1_data[2:] if isinstance(row, dict)]
                    self._b1_source_available = self._validate_b1_rows()
                else:
                    self._b1_source_available = False
            except Exception:
                self._b1_source_available = False
        else:
            self._b1_source_available = False
        ...
        if overrides_path.exists():
            try:
                ...
                for frms_key, cells in data.get("b1_overrides", {}).get("cells_by_bucket", {}).items():
                    for bucket_key, cell in cells.items():
                        self._b1_cells[f"{frms_key}>{bucket_key}"] = cell
                # Repouso mínimo (rest.ranges ou reserva.ranges)
                for source_name in ("rest", "reserva"):
                    source = data.get(source_name, {}) if isinstance(data.get(source_name, {}), dict) else {}
                    for frms_key, ranges in source.get("ranges", {}).items() if isinstance(source.get("ranges", {}), dict) else []:
                        if isinstance(ranges, list):
                            for r in ranges:
                                self._rest_ranges.append({**r, "_frms": frms_key})

                # Faixas de limite de jornada para RESERVA (min/max)
                reserva_data = data.get("reserva", {}) if isinstance(data.get("reserva", {}), dict) else {}
                for frms_key, cfg in reserva_data.items():
                    if not isinstance(cfg, dict) or not frms_key.startswith("FRMS_"):
                        continue
                    min_val = _extract_first_int(cfg.get("min"))
                    max_val = _extract_first_int(cfg.get("max"))
                    if min_val is not None or max_val is not None:
                        self._special_duty_limit_ranges.setdefault("reserva", {})[frms_key] = {
                            "min": min_val,
                            "max": max_val,
                        }
    ...
    def _parse_b1_duration(self, text: str) -> Optional[int]:
        txt = str(text or "").strip()
        if not txt:
            return None
        m = re.match(r"^(\d+)(?:\s*\(\s*(\d+(?:[.,]\d+)?)\s*\))?$", txt)
        if not m:
            return None
        hours = int(m.group(1))
        if m.group(2) is None:
            return hours * 60
        voo_text = m.group(2).replace(",", ".")
        try:
            voo_hours = float(voo_text)
        except ValueError:
            return hours * 60
        return int(round(voo_hours * 60))

    def _find_b1_row(self, hora_local: datetime.time) -> Optional[Dict[str, str]]:
        if not self._b1_rows:
            return None
        hora_min = hora_local.hour * 60 + hora_local.minute
        for row in self._b1_rows:
            range_text = str(row.get("0", "")).strip()
            m = re.match(r"^(\d{2})h(\d{2})-(\d{2})h(\d{2})$", range_text)
            if not m:
                continue
            start = int(m.group(1)) * 60 + int(m.group(2))
            end = int(m.group(3)) * 60 + int(m.group(4))
            if start <= end:
                if start <= hora_min <= end:
                    return row
            else:
                if hora_min >= start or hora_min <= end:
                    return row
        return None

    def _validate_b1_rows(self) -> bool:
        if not self._b1_rows:
            return False
        parsed_ranges = []
        for row in self._b1_rows:
            ...
            if not m:
                return False
            parsed_ranges.append(range_text)
            for column in ["1", "2", "3", "4", "5"]:
                value = str(row.get(column, "")).strip()
                if not value:
                    continue
                if not re.match(r"^\d+(?:\s*\(\s*\d+(?:[.,]\d+)?\s*\))?$", value):
                    return False
        return bool(parsed_ranges)

    def _b1_column_key(self, pousos: int) -> str:
        if pousos <= 2:
            return "1"
        if pousos <= 4:
            return "2"
    ...
    def get_b1_limits(
        ...
        if not self._b1_source_available:
            return None
        if _normalize_text(aclim) != "ACLIMATADO":
            return None
        row = self._find_b1_row(hora_local)
        if row is None:
            return None
        column_key = self._b1_column_key(pousos)
        value = str(row.get(column_key, "")).strip()
        if not value:
            return None
        fdp_match = re.match(r"^(\d+)(?:\s*\(\s*(\d+(?:[.,]\d+)?)\s*\))?$", value)
        if not fdp_match:
            return None
        fdp_hours = int(fdp_match.group(1))
        voo_minutes = None
        if fdp_match.group(2):
            voo_text = fdp_match.group(2).replace(",", ".")
            try:
                voo_minutes = int(round(float(voo_text) * 60))
            except ValueError:
                voo_minutes = None
        return {
            "fdp_max": fdp_hours * 60,
            "voo_max": voo_minutes,
        }
    ...
    def get_special_duty_limit_range(self, special_type: str, frms: str = "OFF") -> Optional[Dict[str, Optional[int]]]:
        """Retorna os valores min/max de jornada para RESERVA a partir do JSON."""
        if _normalize_text(special_type) != "RESERVA":
            return None
        frms_key = f"FRMS_{str(frms).upper()}"
        return self._special_duty_limit_ranges.get("reserva", {}).get(frms_key)
    ...
    def get_fdp_max_minutes(
        ...
        if crew_type == "SIMPLES":
            b1_limits = self.get_b1_limits(hora_local, pousos=pousos, aclim=aclim)
            if b1_limits is not None and b1_limits.get("fdp_max") is not None:
                return b1_limits["fdp_max"]
    ...
    def get_voo_max_minutes(
        ...
        if crew_type == "SIMPLES":
            b1_limits = self.get_b1_limits(hora_local, pousos=pousos, aclim=aclim)
            if b1_limits is not None and b1_limits.get("voo_max") is not None:
                return b1_limits["voo_max"]
...
def _arquivos_diversos_dirs() -> List[pathlib.Path]:
    """Diretórios de apoio (R: preferencial, G: fallback)."""
    dirs: List[pathlib.Path] = []
    for candidate in (ARQUIVOS_DIVERSOS_PATH, ARQUIVOS_DIVERSOS_FALLBACK_PATH):
        if candidate.exists() and candidate not in dirs:
            dirs.append(candidate)
    if not dirs:
        dirs.append(ARQUIVOS_DIVERSOS_PATH)
    return dirs


def _folgas_latam_candidates() -> List[pathlib.Path]:
    names = ["folgas_regulamentares_LATAM.json", "folgas_regulamentares_LATAM"]
    paths: List[pathlib.Path] = []
    seen: Set[str] = set()
    for base in _arquivos_diversos_dirs():
        for name in names:
            p = base / name
            key = str(p).lower()
            if key not in seen:
...
def _folgas_generic_candidates() -> List[pathlib.Path]:
    names = ["folgas_regulamentares.json", "folgas_regulamentares", "folgas.json"]
    paths: List[pathlib.Path] = []
    seen: Set[str] = set()
    for base in _arquivos_diversos_dirs():
        for name in names:
...
def is_latam_company(file_path: pathlib.Path, aeronauta_ctx: Optional[Dict[str, str]] = None) -> bool:
    """Identifica escala LATAM pelo nome do arquivo ou metadados da empresa."""
    if "LATAM" in file_path.name.upper():
        return True
    ctx = aeronauta_ctx or {}
    for key in ("empresa_nome_completo", "empresa_nome_abreviado", "empresa_nome_fantasia"):
        if "LATAM" in _normalize_text(ctx.get(key, "")):
            return True
    return False


def load_folgas_config_union(is_latam: bool) -> Tuple[Set[str], Optional[pathlib.Path]]:
    """
    Carrega atividades de folga conforme a empresa:
      - LATAM: folgas_regulamentares_LATAM.json (exclusivo)
      - demais: união de folgas_regulamentares.json + folgas.json
    """
    merged: Set[str] = set()
    primary_used: Optional[pathlib.Path] = None
    candidates = _folgas_latam_candidates() if is_latam else _folgas_generic_candidates()

    for path in candidates:
        if not path.exists():
            continue
        try:
            loaded = load_folgas_config(path)
            if not loaded:
                continue
    ...
    if is_latam and not merged:
        # Fallback se o JSON LATAM não existir ou estiver vazio.
        for path in _folgas_generic_candidates():
            if not path.exists():
                continue
            try:
                loaded = load_folgas_config(path)
    ...
    return merged, primary_used


def _is_excluded_from_conformity_audit(activity: str) -> bool:
    return _normalize_text(activity) in {_normalize_text(x) for x in ACTIVITIES_EXCLUDED_FROM_CONFORMITY_AUDIT}


def _filter_relevant_entries_for_audit_display(
    entries_data: List[Dict[str, str]],
) -> List[Dict[str, str]]:
    """Remove linhas que não devem aparecer em 'Dados da(s) linha(s) relevante(s)'."""
    filtered: List[Dict[str, str]] = []
    for row in entries_data:
        activity = _normalize_text(row.get("Activity", ""))
        if activity in {_normalize_text(x) for x in ACTIVITIES_EXCLUDED_FROM_CONFORMITY_AUDIT}:
            continue
        filtered.append(row)
    return filtered
...
class ScheduleEntry:
    ...
    def __init__(self,
                 ...
                 tempo_repouso_extra_simples_raw: str = "",
                 tempo_corte_raw: str = "",
                 tempo_solo_raw: str = "",
                 tempo_jornada_raw: str = ""
                 ):
        self.data = data
        ...
        self.tempo_repouso_extra_simples_raw = tempo_repouso_extra_simples_raw
        self.tempo_corte_raw = tempo_corte_raw
        self.tempo_solo_raw = tempo_solo_raw
        self.tempo_jornada_raw = tempo_jornada_raw
...
class ConsecutiveNightDutiesRule(Rule):
    ...
    def check(self, schedule: Schedule, aeronauta_profile: AeronautaProfile) -> List[Violation]:
        ...
        for j_idx, jornada in enumerate(merged_jornadas_trabalho):
            night_date = jornada.get_night_date()
    ...
    def check(self, schedule: Schedule, aeronauta_profile: AeronautaProfile) -> List[Violation]:
        ...
        for idx, entry in enumerate(sorted_entries):
            if not entry.is_folga(aeronauta_profile.folgas_set):
                continue
            if _is_excluded_from_conformity_audit(entry.tipo_atividade):
                continue
...
class CMADispensaExameRule(Rule):
    """Regra: Dispensa para exames médicos (CMA) deve cobrir um dia civil completo (CCT Art. 3.8.4)."""
    def __init__(self):
        super().__init__(
            name="Dispensa para Exame Médico (CMA)",
            description="Verifica se a dispensa para exames médicos (CMA) cobre um dia civil completo (CCT Art. 3.8.4).",
            base_reference="CCT Art. 3.8.4 / Dispensa para Exames Médicos",
            priority=72,
        )
    ...
    def check(self, schedule: Schedule, aeronauta_profile: AeronautaProfile) -> List[Violation]:
        ...
        for idx, entry in enumerate(sorted_entries):
            if entry.tipo_atividade.strip().upper() == "CMA":
                start_local, end_local = entry.get_local_interval(aeronauta_profile)
                duration_minutes = entry.duration_minutes(aeronauta_profile)
                is_full_day = entry.is_full_civil_day(aeronauta_profile)
                
                if duration_minutes < 1440 or not is_full_day:
                    violations.append(
                        Violation(
                            rule_name=self.name,
                            description=(
                                "Atividade CMA (Certificado Médico Aeronauta) inferior a um dia civil completo. "
                                "A CCT concede 01 (um) dia de dispensa para exames médicos periódicos obrigatórios."
                            ),
                            reference=self.base_reference,
                            severity="ALTA",
                            details=(
                                f"Atividade: CMA. Início local: {start_local.strftime('%d/%m/%Y %H:%M')}. "
                                f"Fim local: {end_local.strftime('%d/%m/%Y %H:%M')}. "
                                f"Duração apurada: {duration_minutes // 60:02d}:{duration_minutes % 60:02d}. "
                                f"Conforme a Convenção Coletiva de Trabalho (CCT), Artigo 3.8.4 (Dispensa para exames médicos): "
                                f"\"É concedido 01 (um) dia de dispensa, para o aeronauta fazer os exames médicos periódicos "
                                f"obrigatórios e conforme determinação do órgão oficial competente, sem prejuízo da sua remuneração fixa.\" "
                                f"Dia é um período civil (24 horas), portanto o CMA deve cobrir um dia civil completo (iniciando às 00:01 até 00:00/23:59)."
                            ),
                            relevant_entries_data=[entry.get_report_data()],
                        )
                    )
        return violations
...
class ReportGenerator:
    ...
    def _normalize_csv_duration(value: str) -> str:
        ...
        if not txt or txt.upper() in {"NAN", "NAT", "NONE"}:
            return "N/A"
        minutes = ReportGenerator._parse_csv_duration_to_minutes(txt, default=None)
        if minutes is not None:
            return ReportGenerator._fmt_minutes(minutes)
        # Aceita formatos simples como "12:00" ou "12:00:00"
    ...
    @staticmethod
    def _is_closing_leg(entry: ScheduleEntry) -> bool:
        """Pernas de encerramento (-F / -IF) onde o CSV traz Tempo Repouso e extras."""
        leg = (getattr(entry, "id_leg", "") or "").strip().upper()
        return leg.endswith("-IF") or leg.endswith("-F")

    @staticmethod
    def _parse_csv_duration_to_minutes(value: str, default: Optional[int] = None) -> Optional[int]:
        txt = str(value or "").strip()
        if not txt or txt.upper() in {"NAN", "NAT", "NONE"}:
            return default
        m = re.search(r"(?:(\d+)\s+days?\s+)?(\d{1,3}):(\d{2})(?::\d{2})?$", txt)
        if m:
    ...
    def _repouso_from_closing_legs(
        self,
        entries: List[ScheduleEntry],
        profile: AeronautaProfile,
    ) -> Tuple[Optional[str], Optional[str]]:
        """
        Lê Tempo Repouso e Tempo Repouso Extra Simples das linhas com Id_Leg -F ou -IF.
        Quando há mais de um encerramento no dia, usa o último cronologicamente.
        """
        closing = [e for e in entries if self._is_closing_leg(e)]
        if not closing:
            return None, None
        closing.sort(key=lambda e: e.get_start_datetime(profile))
        last_close = closing[-1]

        horas_prox = self._normalize_csv_duration(getattr(last_close, "tempo_repouso_raw", ""))
        horas_adicionais = self._normalize_csv_duration(
            getattr(last_close, "tempo_repouso_extra_simples_raw", "")
        )
        return (
            horas_prox if horas_prox != "N/A" else None,
            horas_adicionais if horas_adicionais != "N/A" else None,
        )

    def _format_horas_ate_proxima_apresentacao(
        self,
        entries: List[ScheduleEntry],
        profile: AeronautaProfile,
    ) -> str:
        horas_prox, horas_adicionais = self._repouso_from_closing_legs(entries, profile)
        return (
            f"**Horas até a próxima apresentação:** {horas_prox or 'N/A'} "
            f"- **Horas adicionais** {horas_adicionais or 'N/A'}"
        )

    def _extrair_blocos(self, entries: List[ScheduleEntry]) -> List[List[ScheduleEntry]]:
        """Extrai blocos de atividades de uma jornada.

        Um bloco começa com um registro cujo `id_leg` termina em '-I' ou '-IF'
        e termina com um registro cujo `id_leg` termina em '-F' ou '-IF'.
        Em caso de entradas intermediárias sem esses marcadores, elas pertencem ao
        bloco atual.
        """
        if not entries:
            return []

        sorted_entries = sorted(entries, key=lambda e: (e.data, e.hora_inicio, e.hora_fim))

        def _normalize_leg(entry: ScheduleEntry) -> str:
            return ((getattr(entry, 'id_leg', '') or '').strip().upper())

        def _is_start_leg(entry: ScheduleEntry) -> bool:
            leg = _normalize_leg(entry)
            return leg.endswith('-I') or leg.endswith('-IF')

        def _is_final_leg(entry: ScheduleEntry) -> bool:
            leg = _normalize_leg(entry)
            return leg.endswith('-F') or leg.endswith('-IF')

        blocks: List[List[ScheduleEntry]] = []
        cur_block: List[ScheduleEntry] = []

        for entry in sorted_entries:
            if _is_start_leg(entry):
                if cur_block:
                    blocks.append(cur_block)
                cur_block = [entry]
                continue

            if _is_final_leg(entry):
                if not cur_block:
                    cur_block = [entry]
                else:
                    cur_block.append(entry)
                blocks.append(cur_block)
                cur_block = []
                continue
    ...
    def _build_global_blocks(self, all_entries_sorted: List[ScheduleEntry]) -> List[List[ScheduleEntry]]:
        def _normalize_leg(entry: ScheduleEntry) -> str:
            return ((getattr(entry, 'id_leg', '') or '').strip().upper())
    ...
    def _write_special_duty_limits_snapshot(
        ...
        reserva_range = loader.get_special_duty_limit_range("reserva", profile.frms)
        ...
        if reserva_range and reserva_range.get("min") is not None and reserva_range.get("max") is not None:
            f.write(
                f"  • RESERVA | SIMPLES | Jornada: {reserva_range['min']}h - {reserva_range['max']}h\n"
            )
        else:
            f.write(
                f"  • RESERVA | SIMPLES | Jornada: {self._fmt_minutes(reserva_min or 0) if reserva_min is not None else 'N/A'}\n"
            )
        reserva_note = (
        ...
        sobreaviso_line = (
            f"SOBREAVISO | SIMPLES | Jornada: {self._fmt_minutes(sobreaviso_min or 0) if sobreaviso_min is not None else 'N/A'}"
        )
        extras = []
        if standby_ratio:
    ...
    def _write_rest_limits_snapshot(
        ...
        if faixas:
            matched_range = None
            for faixa in faixas:
                fdp_min = faixa.get("fdp_min") or 0
                fdp_max = faixa.get("fdp_max")
                if fdp_max is None:
                    if jornada_minutes >= fdp_min:
                        matched_range = faixa
                        break
                elif fdp_min <= jornada_minutes <= fdp_max:
                    matched_range = faixa
                    break

            if matched_range is not None:
                fdp_min = matched_range.get("fdp_min") or 0
                fdp_max = matched_range.get("fdp_max")
                descanso = matched_range.get("descanso_min")
                if fdp_max is None:
                    faixa_label = f"Jornada superior a {self._fmt_minutes(max(0, fdp_min - 1))}"
                else:
    ...
    def _write_jornada_detail(
        ...
        global_blocks = self._build_global_blocks(all_entries_sorted)
        blocos = [
            block for block in global_blocks
            if any(e in entries for e in block)
        ]

        if not blocos:
            return

        # Para pré/post e jornada total, considere o primeiro e último de todas as entradas
        all_sorted = sorted(entries, key=lambda e: e.get_start_datetime(profile))
        first_entry = all_sorted[0]
        last_entry = all_sorted[-1]

        jornada_minutes = jornada.duracao_jornada_minutos()
        # Contagem inicial de pousos (usada para cálculo de limites).
        # Use the jornada-level count instead of the optional `is_pouso` attribute,
        # pois o CSV não garante esse campo para todas as entradas.
        pousos = jornada.numero_pousos()
        ...
        def _block_marker(entry: ScheduleEntry) -> str:
            leg = (getattr(entry, 'id_leg', '') or '').strip().upper()
            if leg.endswith('-IF'):
                return '-IF'
            if leg.endswith('-I'):
        ...
        def _block_route(entry: ScheduleEntry) -> str:
            leg = (getattr(entry, 'id_leg', '') or '').strip()
            m = re.match(r'^(.*?)(?:-(?:I|F|IF|M))?$', leg, re.IGNORECASE)
            return m.group(1) if m else leg

        f.write(f"- **Blocos:**\n")
        total_blocks_minutes = 0
        total_flight_minutes = 0
        total_pousos = 0
        for idx, bloco in enumerate(blocos, start=1):
            block_minutes = sum(e.duration_minutes(profile) for e in bloco)
            flight_minutes_block = sum(e.duration_minutes(profile) for e in bloco if self._is_voo_atividade(e, profile))
            pousos_block = sum(1 for e in bloco if self._is_voo_atividade(e, profile))
            total_blocks_minutes += block_minutes
            total_flight_minutes += flight_minutes_block
            total_pousos += pousos_block

            markers = "+".join(_block_marker(e) for e in bloco)
            route = f"{_block_route(bloco[0])}->{_block_route(bloco[-1])}"
            f.write(f"  • {markers} {route} - = **{self._fmt_minutes(block_minutes)}**\n")

        # Resumo agregado dos blocos do dia
        block_minutes = total_blocks_minutes
        flight_minutes = total_flight_minutes
        pousos = total_pousos

        def _solo_label(entry: ScheduleEntry) -> str:
            activity = str(getattr(entry, 'tipo_atividade', '') or '').strip()
            if activity:
                return activity
            if getattr(entry, 'descricao', '').strip():
                return entry.descricao.strip()
            return str(getattr(entry, 'voo_numero', '') or 'N/A')

        def _is_final_leg(entry: ScheduleEntry) -> bool:
            leg = (getattr(entry, 'id_leg', '') or '').strip().upper()
            return leg.endswith('-F') or leg.endswith('-IF')

        last_block_entry = blocos[-1][-1] if blocos else last_entry
        jornada_raw = self._normalize_csv_duration(getattr(last_block_entry, 'tempo_jornada_raw', ''))
        jornada_display = jornada_raw if jornada_raw != 'N/A' else self._fmt_minutes(jornada_minutes)
        jornada_minutes_for_rest = self._parse_csv_duration_to_minutes(jornada_raw, jornada_minutes)

        f.write(f"- **Solo entre pernas:**\n")
        solo_entries = [entry for bloco in blocos for entry in bloco if not _is_final_leg(entry)]
        if solo_entries:
            for idx, entry in enumerate(solo_entries):
                solo_time = self._normalize_csv_duration(getattr(entry, 'tempo_solo_raw', ''))
                if solo_time == 'N/A' and idx + 1 < len(solo_entries):
                    try:
                        next_entry = solo_entries[idx + 1]
                        gap_minutes = int((next_entry.get_start_datetime(profile) - entry.get_end_datetime(profile)).total_seconds() / 60)
                    except Exception:
                        gap_minutes = 0
                    solo_time = self._fmt_minutes(gap_minutes)
                f.write(f"  • {_solo_label(entry)}: -> = **{solo_time}**\n")
        else:
            f.write(f"  • ****: -> = ****\n")
        ...
        # Exibe Tempo Corte apenas se houver id_leg terminando em '_IF' ou '-F' no bloco
        final_leg_entry = None
        for bloco in reversed(blocos):
            for entry in reversed(bloco):
                if _is_final_leg(entry):
                    final_leg_entry = entry
                    break
            if final_leg_entry:
                break

        if final_leg_entry:
            _corte_raw = final_leg_entry.tempo_corte_raw if final_leg_entry.tempo_corte_raw else ''
            try:
        ...
        f.write(f"**Jornada do dia (apresentação->liberação):** {jornada_display}\n")
        bloco_entries = [e for bloco in blocos for e in bloco]
        f.write(f"{self._format_horas_ate_proxima_apresentacao(bloco_entries, profile)}\n\n")

        self._write_limits_snapshot(f, profile, loader, hora_apres_local, pousos)
        self._write_rest_limits_snapshot(f, profile, loader, jornada_minutes_for_rest)
        ...
        if flight_minutes > 0:
            f.write("- OBS: Os valores de Tempo de voo, Jornada do dia, Horas até a próxima apresentação e Horas adicionais foram reportados acima; confira com os limites disponibilizados.\n")
        else:
    ...
    def _write_non_work_day_detail(
        ...
        f.write("**Horas até a próxima apresentação:** N/A - **Horas adicionais** N/A\n\n")
            ...
            if not violations:
                ...
                for rule_name, rule_violations in display_violations_by_rule.items():
                    ...
                    for v in rule_violations:
                        ...
                        relevant_rows = _filter_relevant_entries_for_audit_display(v.relevant_entries_data)
                        if relevant_rows:
                            f.write("    Dados da(s) linha(s) relevante(s):\n")
                            headers = ["Activity", "Id_Leg", "Checkin", "Start", "Dep", "Arr", "End", "Checkout"]
                            col_widths = {header: len(header) for header in headers}
                            for entry_data in relevant_rows:
...
def parse_schedule_csv(
    ...
    for col in required_datetime_cols:
        if col in df.columns:
            ...
            if missing_mask.any():
                parsed_fallback = pd.to_datetime(df.loc[missing_mask, col], errors='coerce', dayfirst=True)
    ...
    text_cols = [
        'tipo_atividade', 'local_inicio', 'local_fim', 'descricao', 'voo_numero', 'id_leg_csv',
        'tempo_repouso', 'tempo_repouso_extra_simples', 'tempo_solo', 'tempo_jornada',
    ]
    ...
    for i, row in df.iterrows():
        ...
        entry = ScheduleEntry(
            ...
            tempo_repouso_extra_simples_raw=(
                row['tempo_repouso_extra_simples']
                if 'tempo_repouso_extra_simples' in row and pd.notna(row['tempo_repouso_extra_simples'])
                else ''
            ),
            tempo_corte_raw=str(row['tempo_corte']) if 'tempo_corte' in row and pd.notna(row['tempo_corte']) else '',
            tempo_solo_raw=row['tempo_solo'] if 'tempo_solo' in row and pd.notna(row['tempo_solo']) else '',
            tempo_jornada_raw=row['tempo_jornada'] if 'tempo_jornada' in row and pd.notna(row['tempo_jornada']) else ''
        )
...
def main():
    # print("--- Auditor de Escalas de Aeronautas ---")

    # Permite execução sem diálogo quando chamado pelo orquestrador.
    input_path_str = os.getenv("AERO_ESCALA_CSV", "").strip().strip('"')
    sem_popup = os.getenv("AERO_NO_POPUP", "").strip().lower() in {"1", "true", "yes", "sim"}

    if not input_path_str:
        # Seleciona arquivo de entrada via diálogo
        root = tk.Tk()
        root.withdraw()
        root.attributes('-topmost', True)

        input_path_str = filedialog.askopenfilename(
            title="Selecione o arquivo da escala (CSV)",
            filetypes=[("Arquivos CSV", "*.csv"), ("Todos os arquivos", "*.*")]
        )
        root.destroy()

    if sem_popup and not input_path_str:
        # print("Execução sem popup ativa e nenhum CSV informado em AERO_ESCALA_CSV.")
        return
    ...
    input_file_path = pathlib.Path(input_path_str)

    if not input_file_path.exists():
        # print(f"Erro: O arquivo '{input_file_path}' não foi encontrado.")
        return
    elif not input_file_path.is_file():
        # print(f"Erro: O caminho '{input_file_path}' não aponta para um arquivo.")
        return
    elif input_file_path.suffix.lower() != '.csv':
        # print(f"Erro: O arquivo '{input_file_path}' não é um arquivo CSV.")
        return

    preferred_input_file_path = _resolve_preferred_input_csv(input_file_path)
    if preferred_input_file_path != input_file_path:
        # print(
            # "Aviso: foi localizada uma versão mais avançada da mesma escala. "
            # f"O relatório será gerado a partir de '{preferred_input_file_path.name}' "
            # f"em vez de '{input_file_path.name}'."
        # )
        input_file_path = preferred_input_file_path

    # Carrega metadados das empresas para identificar empresa/função a partir do nome do CSV
    try:
        company_metadata = load_company_metadata(METADATA_EOS_PATH)
    except Exception as e:
        # print(f"Aviso: não foi possível carregar metadata_eos.json: {e}")
        company_metadata = {"records": [], "by_abbrev": {}, "by_name": {}, "by_coa": {}}

    aeronauta_ctx = parse_aeronauta_context_from_filename(input_file_path, company_metadata)

    # Define o caminho de saída para o relatório
    output_dir = input_file_path.parent
    output_stem = input_file_path.stem
    output_report_filename = f"{output_stem} RELATORIO CONFORMIDADE.txt"
    output_report_path = output_dir / output_report_filename

    # --- Carrega configuração de Folgas (R: preferencial) ---
    is_latam_source = is_latam_company(input_file_path, aeronauta_ctx)
    folgas_set, _folgas_source_path = load_folgas_config_union(is_latam_source)
    if not folgas_set:
        # print("Aviso: nenhuma atividade de folga carregada dos JSONs em Arquivos_Diversos.")
        pass

    # --- Catálogo de atividades LATAM ---
    base_folgas_dir = next(iter(_arquivos_diversos_dirs()), ARQUIVOS_DIVERSOS_PATH)
    latam_activities_set: Set[str] = set()
    latam_activity_groups: Dict[str, Set[str]] = {"reserva": set(), "sobreaviso": set()}
    if is_latam_source:
        latam_activity_candidates = [
            base_folgas_dir / "AtividadesEscalaLATAM",
            base_folgas_dir / "AtividadesEscalaLATAM.json",
            ATIVIDADES_ESCALA_LATAM_PATH,
        ]
        latam_activity_path = next((p for p in latam_activity_candidates if p.exists()), latam_activity_candidates[-1])
        try:
            latam_activities_set = load_latam_activities_config(latam_activity_path)
            latam_activity_groups = load_latam_activity_groups(latam_activity_path)
            # print(
                # f"Atividades LATAM carregadas de '{latam_activity_path.name}': "
                # f"{len(latam_activities_set)} token(s)."
            # )
            # print(
                # f"Grupos LATAM: reserva={len(latam_activity_groups.get('reserva', set()))} | "
                # f"sobreaviso={len(latam_activity_groups.get('sobreaviso', set()))}."
            # )
        except Exception as e:
            # print(f"Erro ao carregar catálogo de atividades LATAM: {e}")
            # print("Continuando sem catálogo LATAM de referência.")

            pass
    # --- Dados do Aeronauta (Exemplo - Em um sistema real, viriam do Supabase) ---
    # Estes dados devem ser dinâmicos e associados ao aeronauta logado
    # Para o teste, você pode ajustar esses valores
    aeronauta_profile = AeronautaProfile(
        nome=aeronauta_ctx["nome"],
        matricula=aeronauta_ctx["matricula"],
        base_domiciliar=aeronauta_ctx["base"],
        fuso_base="America/Sao_Paulo",          # Fuso horário de São Paulo/Campinas
        tipo_aeronave_principal="JATO",
        funcao=aeronauta_ctx["funcao"],
        cct_aplicavel="CCT_AERONAUTAS_2024_2025",
        crew_type="SIMPLES",                    # "SIMPLES" | "AUMENTADA" | "COMPLEMENTADA"
        frms="ON" if _normalize_text(aeronauta_ctx.get("empresa_gestao_fadiga", "")) in {"SIM", "ON", "TRUE", "1"} else "OFF",
        aclimatado="ACLIMATADO"                 # "ACLIMATADO" | "NAO_ACLIMATADO"
        ,
        empresa_nome_completo=aeronauta_ctx["empresa_nome_completo"],
        empresa_nome_abreviado=aeronauta_ctx["empresa_nome_abreviado"],
        empresa_nome_fantasia=aeronauta_ctx["empresa_nome_fantasia"],
        empresa_cnpj=aeronauta_ctx["empresa_cnpj"],
        empresa_rbac_tipo=aeronauta_ctx["empresa_rbac_tipo"],
        empresa_tipo_operacao=aeronauta_ctx["empresa_tipo_operacao"],
        empresa_gestao_fadiga=aeronauta_ctx["empresa_gestao_fadiga"]
    )
    # Adiciona o set de folgas ao perfil do aeronauta para ser acessível pelas regras
    aeronauta_profile.folgas_set = folgas_set 
    aeronauta_profile.latam_activities_set = latam_activities_set
    aeronauta_profile.latam_reserva_codes_set = latam_activity_groups.get("reserva", set())
    aeronauta_profile.latam_sobreaviso_codes_set = latam_activity_groups.get("sobreaviso", set())

    # print(
        # f"\nPerfil do Aeronauta carregado: {aeronauta_profile.nome} "
        # f"({aeronauta_profile.funcao} - {aeronauta_profile.base_domiciliar}) "
        # f"[Empresa: {aeronauta_profile.empresa_nome_abreviado or aeronauta_profile.empresa_nome_completo}, "
        # f"FRMS={aeronauta_profile.frms}, Aclimatado={aeronauta_profile.aclimatado}]."
    # )

    # --- 7. Carregar e Processar a Escala ---
    # print(f"Processando escala do arquivo: '{input_file_path.name}'...")
    audit_year_month = infer_audit_month_from_filename(input_file_path)
    if audit_year_month is not None:
        y, m = audit_year_month
        # print(f"Mês de auditoria inferido do arquivo: {m:02d}/{y}.")
    else:
        # print("Não foi possível inferir o mês de auditoria pelo nome do arquivo. Será considerado todo o CSV.")

        pass
    try:
        schedule = parse_schedule_csv(input_file_path, aeronauta_profile, audit_year_month=audit_year_month)
        if audit_year_month is not None and len(schedule.all_entries) == 0:
            # print(
                # "Aviso: filtro mensal inferido resultou em 0 registros. "
                # "Reprocessando CSV sem filtro de mês."
            # )
            schedule = parse_schedule_csv(input_file_path, aeronauta_profile, audit_year_month=None)
        # print(f"Escala carregada com {len(schedule.jornadas)} jornadas de trabalho e {len(schedule.all_entries)} entradas totais (incluindo folgas/repousos).")
    except Exception as e:
        sys.exit(1)

    # --- 8. Configurar Regras de Auditoria ---
    # Os limites operacionais são carregados automaticamente do JSON:
    #   G:/SPECTRUM_SYSTEM/Aeronautas/Documentos_Comuns/Documentacao_Oficial/regras_json/
    #   → legal_overrides.json  (b1_overrides: FDP máx por horário/pousos; rest.ranges: repouso mínimo)
    #   → rbac117_rulepack.json (fallback)
    # Para sobrescrever com limites CCT, passe cct_limit_hours / cct_min_rest_hours.
    rules_to_apply = [
        DailyDutyLimitRule(aeronauta_profile.tipo_aeronave_principal, aeronauta_profile.funcao, cct_limit_hours=None),
        MinimumRestPeriodRule(aeronauta_profile.tipo_aeronave_principal, aeronauta_profile.funcao, cct_min_rest_hours=None),
        ConsecutiveNightDutiesRule(max_consecutive=2, max_total_168h=4, cct_max_consecutive=None, cct_max_total_168h=None),
        MonthlyAnnualFlightHoursRule(),
        CivilDayFolgaRule(),
        MonthlyFolgasRule(),
        WeekendConsecutiveFolgasRule(),
        GroupedFolgasRule(),
        SixthPeriodRule(),
        CMADispensaExameRule(),
    ]
    auditor = Auditor(rules_to_apply)

    # --- 9. Realizar a Auditoria ---
    # print("Iniciando auditoria da escala...")
    violations = auditor.audit_schedule(schedule, aeronauta_profile)
    # print(f"Auditoria concluída. Encontradas {len(violations)} não conformidades.")

    # --- 10. Gerar o Relatório ---
    # print(f"Gerando relatório de conformidade em: '{output_report_path.name}'...")
    report_generator = ReportGenerator(input_file_path.name, aeronauta_ctx)
    try:
        report_generator.generate_report(aeronauta_profile, schedule, violations, output_report_path)
        # print("Relatório gerado com sucesso!")
        # print(f"Você pode encontrar o relatório em: {output_report_path}")
    except Exception as e:
        sys.exit(1)
