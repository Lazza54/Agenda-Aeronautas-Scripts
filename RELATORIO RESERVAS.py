...
"""
Gerador de Relatório de Explorar Reservas
...
class RelatorioReservas:
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        """Gera relatório informando que não há dados na operação"""
        ...
        # Criar tabela com fundo vermelho e mensagem
        aviso_data = [['NÃO HÁ DADOS NESTA OPERAÇÃO']]
        aviso_table = Table(aviso_data, colWidths=[18*cm])
        ...
        # Informações do aeronauta se disponíveis
        ...
        try:
            doc.build(story)
            print(f"Relatório vazio gerado com sucesso: {nome_arquivo}")
            return True
        except Exception as e:
        ...
        for coluna in colunas_tempo:
            ...
            for registro in registros:
                if coluna in registro and not pd.isna(registro[coluna]):
                    ...
                    if coluna != 'Tempo Reserva':  # Evitar contar duplicado
                        total_geral += registro[coluna]
            sumario[coluna] = total
        
        # Se não há total calculado, usar o campo Tempo Reserva
        if total_geral == pd.Timedelta(0):
            for registro in registros:
                if 'Tempo Reserva' in registro and not pd.isna(registro['Tempo Reserva']):
                    total_geral += registro['Tempo Reserva']
        
        sumario['Tempo Reserva Total'] = total_geral
        return sumario
    ...
    def criar_tabela_registros(self, registros, styles):
        ...
        for registro in registros:
            ...
            # Calcular total do tempo de reserva
            tempo_total = pd.Timedelta(0)
            if 'Tempo Reserva' in registro and not pd.isna(registro['Tempo Reserva']):
                tempo_total = registro['Tempo Reserva']
            else:
                # Fallback: somar as categorias
                for col in ['Tempo Reserva Diurno', 'Tempo Reserva Noturno', 
        ...
        # Criação da tabela
        # 8 colunas: Atividade (2.2cm), Apresentação (2.6cm), Partida (2.6cm), De (1.3cm), Para (1.3cm), Fim (2.6cm), Chegada (2.6cm), Tempo Reserva Total (2.8cm) = 18cm
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas de Reserva - TOTAL', self.formatar_tempo(sumario.get('Tempo Reserva Total', pd.Timedelta(0)))],
            ['Horas de Reserva Diurnas', self.formatar_tempo(sumario.get('Tempo Reserva Diurno', pd.Timedelta(0)))],
            ['Horas de Reserva Noturnas', self.formatar_tempo(sumario.get('Tempo Reserva Noturno', pd.Timedelta(0)))],
            ['Horas de Reserva Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Reserva Especial Diurno', pd.Timedelta(0)))],
            ['Horas de Reserva Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Reserva Especial Noturno', pd.Timedelta(0)))]
        ]
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_reserva.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE HORAS DE EXPLORAR RESERVA", titulo_style))
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE EXPLORAR RESERVA")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Explorar Reserva...")
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV de Explorar Reserva",
        filetypes=[
            ("Arquivos CSV", "*.csv"),
            ("Todos os arquivos", "*.*")
        ]
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioReservas(arquivo_csv)
    ...
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_RESERVA_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
