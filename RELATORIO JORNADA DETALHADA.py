# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Jornada - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de jornada divididas por períodos
"""
...
class RelatorioJornadaDetalhada:
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_jornada_detalhada.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE JORNADA", titulo_style))
        ...
        total_diurno_geral = self.df['Tempo Jornada Diurno'].sum()
        total_noturno_geral = self.df['Tempo Jornada Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Jornada Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Jornada Especial Noturno'].sum()
        ...
        if len(anos) > 0:
            ...
            for ano in anos:
                ...
                indice_data.append([
                    ...
                    Paragraph(f'Detalhamento de Jornada - {ano_int}', styles['Normal'])
                ])
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Jornada Diurno'].sum()
            total_noturno = df_ano['Tempo Jornada Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Jornada Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Jornada Especial Noturno'].sum()
            ...
            detalhes_data = [['DATA', 'ATIVIDADE', 'DIURNO', 'NOTURNO', 
                              'ESPECIAL\nDIURNO', 'ESPECIAL\nNOTURNO', 'TOTAL']]
            ...
            for idx, row in df_ano.iterrows():
                ...
                if 'Activity' in self.df.columns:
                    atividade = str(row.get('Activity', ''))
                
                d = self._formatar_timedelta(row['Tempo Jornada Diurno'])
                n = self._formatar_timedelta(row['Tempo Jornada Noturno'])
                ed = self._formatar_timedelta(row['Tempo Jornada Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Jornada Especial Noturno'])
                
                total_linha = (row['Tempo Jornada Diurno'] + 
                              (row['Tempo Jornada Noturno'] * 2) +
                              (row['Tempo Jornada Especial Diurno'] * 2) + 
                              (row['Tempo Jornada Especial Noturno'] * 3))
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE JORNADA")
...
def main():
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV PROCESSADO de Jornada (com tempos calculados)",
        filetypes=[("Arquivos CSV", "*JORNADA*.csv"), ("Todos os arquivos", "*.csv")],
        parent=root
    )
    ...
    # Gerar relatório
    relatorio = RelatorioJornadaDetalhada(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_HORAS_JORNADA {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
