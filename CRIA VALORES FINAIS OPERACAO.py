...
def gerar_nome_csv_saida(nome_csv_entrada: str) -> str:
    """
    Gera o nome do arquivo CSV de saída, substituindo '_QUARTA_VERSAO' por '_OPERACAO'
    e desprezando qualquer texto após '_QUARTA_VERSAO'.
    """
    ...
    if '_QUARTA_VERSAO' in nome_sem_ext:
        ...
        return f"{prefix}_OPERACAO_{datetime.now().strftime('%d%m%Y_%H%M%S')}.csv"
    
    # Fallback para outros casos: se '_QUARTA_VERSAO' não for encontrado,
    # tenta remover o timestamp e adicionar '_OPERACAO' se ainda não tiver
    timestamp_pattern = r'_\d{8}_\d{6}$' # Exemplo: _YYYYMMDD_HHMMSS
    cleaned_name = re.sub(timestamp_pattern, '', nome_sem_ext)
    
    # Se o nome já terminar com _OPERACAO, não adiciona novamente
    if not cleaned_name.endswith('_OPERACAO'):
        return f"{cleaned_name}_OPERACAO_{datetime.now().strftime('%d%m%Y_%H%M%S')}.csv"
    
    return f"{cleaned_name}_{datetime.now().strftime('%d%m%Y_%H%M%S')}.csv" # Caso já termine com _OPERACAO
...
def parse_date_from_excel_serial(serial_value):
    ...
    # Excel no Windows usa 1899-12-30 como dia 0. No Mac, é 1904-01-01.
    # Assumindo padrão Windows.
...
def load_json_to_set(file_name: str, type_expected: str = 'string') -> set:
    ...
    'type_expected' pode ser 'string' (para folgas) ou 'date' (para feriados).
    """
...
def calculate_all_durations(start_dt: datetime, end_dt: datetime, holidays_set: set) -> tuple:
    """
    Calcula os tempos diurnos/noturnos (normais e especiais) para operação e pagamento
    de um determinado intervalo (start_dt, end_dt).
    ...
    # Operação: Noturno 18:00 - 06:00
    op_night_total = get_interval_overlap_with_repeating_period(start_dt, end_dt, 18, 0, 6, 0)
    op_day_total = (end_dt - start_dt) - op_night_total

    # Pagamento: Noturno 21:00 - 09:00
    pay_night_total = get_interval_overlap_with_repeating_period(start_dt, end_dt, 21, 0, 9, 0)
    pay_day_total = (end_dt - start_dt) - pay_night_total

    # --- Cálculo de Tempos Diurno/Noturno Especiais ---
    op_special_day = timedelta(0)
    op_special_night = timedelta(0)
    ...
    while current_dt < end_dt:
        ...
        if eh_especial:
            # Operação: verificar se é noturno (18:00-06:00)
            if current_hour >= 18 or current_hour < 6:
                op_special_night += delta
            else:
                op_special_day += delta
    ...
    return (op_day_total, op_night_total, op_special_day, op_special_night,
...
def processar_dados_aeronautica():
    ...
    try:
        # Carrega folgas.json para filtragem inicial (strings normalizadas)
    ...
    try:
        ...
        column_map_input_to_internal = {
            ...
            'Checkout': 'checkout', 'Tempo Operacao': 'tempo_operacao',
    ...
    # Recalcular Tempo Operacao para garantir consistência e tipo Timedelta após conversão de datas
    df['tempo_operacao'] = df['end'] - df['start']
    # Preenche NaT (erros na subtração) com Timedelta(0)
    df['tempo_operacao'] = df['tempo_operacao'].fillna(pd.Timedelta(0))

    print("Colunas de data/hora convertidas e Tempo Operacao recalculado.")
    print("Amostra de colunas de tempo após conversão:")
    print(df[['start', 'end', 'tempo_operacao']].head().to_string())

    # --- 5. Filtragem por folgas.json (Corrigido) ---
    # Normalizar activity para facilitar comparações com os sets
    df['activity_norm'] = df['activity'].astype(str).str.strip().str.upper()

    initial_row_count = len(df)
    # Filtrar linhas onde 'activity_norm' NÃO está em 'folgas_data'
    if 'activity_norm' in df.columns and folgas_data:
        df_processed = df[~df['activity_norm'].isin(folgas_data)].copy()
        print(f"Total de linhas lidas: {initial_row_count}. Linhas após filtrar folgas: {len(df_processed)}")
        if initial_row_count - len(df_processed) > 0:
            print(f"{initial_row_count - len(df_processed)} linhas foram removidas pela filtragem de folgas.")
        else:
            print("Nenhuma linha removida pela filtragem de folgas.")
    else:
        print("Aviso: Coluna 'activity' ou dados de folga ausentes/vazios. Nenhuma filtragem por folga será aplicada.")
        df_processed = df.copy() # Processa todas as linhas

    # Remova a coluna temporária 'activity_norm'
    df_processed.drop(columns=['activity_norm'], inplace=True, errors='ignore')

    # --- 6. Adicionar e Calcular as 8 Novas Colunas (Corrigido) ---
    new_internal_columns = [
        'tempo_operacao_diurno', 'tempo_operacao_noturno',
        'tempo_operacao_especial_diurno', 'tempo_operacao_especial_noturno',
    ...
    print("Calculando os novos tempos de operação e pagamento...")
    
    # Aplica a função de cálculo a cada linha
    # Verifica explicitamente se 'start' e 'end' são NaT e se 'start' é anterior a 'end'
    valid_rows_mask = df_processed['start'].notna() & df_processed['end'].notna() & (df_processed['start'] < df_processed['end'])
    
    # Aplica a função apenas nas linhas válidas
    results = df_processed.loc[valid_rows_mask].apply(
        lambda row: calculate_all_durations(row['start'], row['end'], feriados_data),
        axis=1,
        result_type='expand' 
    )
    ...
    # --- 7. Preparar DataFrame para Saída (Ordem Corrigida) ---
    # Define as colunas desejadas no CSV de saída com seus nomes exatos e na ordem correta
    final_output_columns_pascal_case = [
        'Activity', 'Id_Leg', 'Checkin', 'Start', 'Dep', 'Arr', 'End', 'Checkout', 'Tempo Operacao',
        'Tempo Operacao Diurno', 'Tempo Operacao Noturno',
        'Tempo Operacao Especial Diurno', 'Tempo Operacao Especial Noturno',
    ...
    internal_to_output_name_map = {
        ...
        'checkout': 'Checkout', 'tempo_operacao': 'Tempo Operacao',
        'tempo_operacao_diurno': 'Tempo Operacao Diurno',
        'tempo_operacao_noturno': 'Tempo Operacao Noturno',
        'tempo_operacao_especial_diurno': 'Tempo Operacao Especial Diurno',
        'tempo_operacao_especial_noturno': 'Tempo Operacao Especial Noturno',
    ...
    timedelta_cols_for_format = [
        'Tempo Operacao', 'Tempo Operacao Diurno', 'Tempo Operacao Noturno',
        'Tempo Operacao Especial Diurno', 'Tempo Operacao Especial Noturno',
        'Pagamento Diurno', 'Pagamento Noturno',
        'Pagamento Especial Diurno', 'Pagamento Especial Noturno'
    ]
    ...
    # Formatar colunas datetime para string 'YYYY-MM-DD HH:MM:SS'
    ...
    try:
        ...
        print(f"Arquivo '{output_filename}' salvo com sucesso em: {output_file_path}")
