...
"""
Gerador de Relatório de Horas Após Corte
...
class RelatorioAposCorte:
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            # Calcular Tempo Após Corte como Checkout - End
            if 'Checkout' in self.df.columns and 'End' in self.df.columns:
                self.df['Tempo Corte Calculado'] = self.df['Checkout'] - self.df['End']
    ...
    def calcular_sumario(self, registros):
        ...
        sumario = {}
        # Calcular total geral somando todas as categorias
        ...
        sumario['Tempo Corte Total'] = total_geral
        return sumario
    ...
    def criar_tabela_registros(self, registros, styles):
        ...
        for registro in registros:
            ...
            # Calcular total das horas após corte para este registro
            # Usar o cálculo direto: Checkout - End
            tempo_total = pd.Timedelta(0)
            if 'Tempo Corte Calculado' in registro and not pd.isna(registro['Tempo Corte Calculado']):
                tempo_total = registro['Tempo Corte Calculado']
            else:
                # Fallback: somar as categorias
                for col in ['Tempo Corte Diurno', 'Tempo Corte Noturno', 
        ...
        # Criação da tabela
        # 8 colunas: Atividade (2.2cm), Apresentação (2.6cm), Partida (2.6cm), De (1.3cm), Para (1.3cm), Fim (2.6cm), Chegada (2.6cm), Tempo Após Corte (2.8cm) = 18cm
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas Após Corte - TOTAL', self.formatar_tempo(sumario.get('Tempo Corte Total', pd.Timedelta(0)))],
            ['Horas Após Corte Diurnas', self.formatar_tempo(sumario.get('Tempo Corte Diurno', pd.Timedelta(0)))],
            ['Horas Após Corte Noturnas', self.formatar_tempo(sumario.get('Tempo Corte Noturno', pd.Timedelta(0)))],
            ['Horas Após Corte Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Corte Especial Diurno', pd.Timedelta(0)))],
            ['Horas Após Corte Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Corte Especial Noturno', pd.Timedelta(0)))]
        ]
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_corte.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE HORAS APÓS CORTE", titulo_style))
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE HORAS APÓS CORTE")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Após Corte...")
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV de Após Corte",
        filetypes=[
            ("Arquivos CSV", "*.csv"),
            ("Todos os arquivos", "*.*")
        ]
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioAposCorte(arquivo_csv)
    
    # Define nome do arquivo de saída
    diretorio = os.path.dirname(arquivo_csv)
    nome_base = os.path.splitext(os.path.basename(arquivo_csv))[0]
    nome_base = nome_base.replace("_TEMPO_CORTE_", "_CORTE_")
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_CORTE_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
