...
class RelatorioPlantao:
        ...
        for coluna in colunas_tempo:
            ...
            for registro in registros:
                if coluna in registro and not pd.isna(registro[coluna]):
                    ...
                    if coluna != 'Tempo Plantao':  # Evitar contar duplicado
                        total_geral += registro[coluna]
            sumario[coluna] = total
        
        # Se não há total calculado, usar o campo Tempo Plantao
        if total_geral == pd.Timedelta(0):
            for registro in registros:
                if 'Tempo Plantao' in registro and not pd.isna(registro['Tempo Plantao']):
                    total_geral += registro['Tempo Plantao']
        
        sumario['Tempo Plantao Total'] = total_geral
        return sumario
    ...
    def criar_tabela_registros(self, registros, styles):
        ...
        for registro in registros:
            ...
            # Calcular total do tempo de plantão
            tempo_total = pd.Timedelta(0)
            if 'Tempo Plantao' in registro and not pd.isna(registro['Tempo Plantao']):
                tempo_total = registro['Tempo Plantao']
            else:
                # Fallback: somar as categorias
                for col in ['Tempo Plantao Diurno', 'Tempo Plantao Noturno', 
        ...
        # Criação da tabela
        # 8 colunas: Atividade (2.2cm), Apresentação (2.6cm), Partida (2.6cm), De (1.3cm), Para (1.3cm), Fim (2.6cm), Chegada (2.6cm), Tempo Plantão Total (2.8cm) = 18cm
        tabela = Table(dados_tabela, colWidths=[2.2*cm, 2.6*cm, 2.6*cm, 1.3*cm, 1.3*cm, 2.6*cm, 2.6*cm, 2.8*cm])
        
        # Estilo da tabela (igual ao RELATORIO OPERACAO)
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas de Plantão - TOTAL', self.formatar_tempo(sumario.get('Tempo Plantao Total', pd.Timedelta(0)))],
            ['Horas de Plantão Diurnas', self.formatar_tempo(sumario.get('Tempo Plantao Diurno', pd.Timedelta(0)))],
            ['Horas de Plantão Noturnas', self.formatar_tempo(sumario.get('Tempo Plantao Noturno', pd.Timedelta(0)))],
            ['Horas de Plantão Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Plantao Especial Diurno', pd.Timedelta(0)))],
            ['Horas de Plantão Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Plantao Especial Noturno', pd.Timedelta(0)))]
        ]
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DE HORAS DE PLANTÃO")
            ...
            # Marca d'água diagonal "SEM REGISTROS LANÇADOS"
            c.saveState()
            c.translate(width/2, height/2)
            c.rotate(45)
            c.setFont("Helvetica-Bold", 20)
            c.setFillColorRGB(1, 0, 0, alpha=0.3)  # Vermelho com transparência
            text = "SEM REGISTROS LANÇADOS"
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_plantao.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE HORAS DE PLANTÃO", titulo_style))
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE PLANTÃO")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Plantão...")
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV de Plantão",
        filetypes=[
            ("Arquivos CSV", "*.csv"),
            ("Todos os arquivos", "*.*")
        ]
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioPlantao(arquivo_csv)
    ...
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_PLANTAO_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
