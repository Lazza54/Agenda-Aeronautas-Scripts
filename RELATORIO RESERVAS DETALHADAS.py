# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Explorar Reserva - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de reserva divididas por períodos
"""
...
class RelatorioReservasDetalhadas:
    ...
    def _formatar_timedelta(self, td):
        """Converte timedelta ou string para formato HH:MM"""
        import pandas as pd
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_reservas_detalhadas.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE EXPLORAR RESERVA", titulo_style))
        ...
        total_diurno_geral = self.df['Tempo Reserva Diurno'].sum()
        total_noturno_geral = self.df['Tempo Reserva Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Reserva Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Reserva Especial Noturno'].sum()
        ...
        if len(anos) > 0:
            ...
            for ano in anos:
                ...
                indice_data.append([
                    ...
                    Paragraph(f'Detalhamento de Explorar Reserva - {ano_int}', styles['Normal'])
                ])
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Reserva Diurno'].sum()
            total_noturno = df_ano['Tempo Reserva Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Reserva Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Reserva Especial Noturno'].sum()
            ...
            for idx, row in df_ano.iterrows():
                ...
                d = self._formatar_timedelta(row['Tempo Reserva Diurno'])
                n = self._formatar_timedelta(row['Tempo Reserva Noturno'])
                ed = self._formatar_timedelta(row['Tempo Reserva Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Reserva Especial Noturno'])
                
                total_linha = (row['Tempo Reserva Diurno'] + 
                              (row['Tempo Reserva Noturno'] * 2) +
                              (row['Tempo Reserva Especial Diurno'] * 2) + 
                              (row['Tempo Reserva Especial Noturno'] * 3))
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE EXPLORAR RESERVA")
...
def main():
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV PROCESSADO de Explorar Reserva (com tempos calculados)",
        filetypes=[("Arquivos CSV", "*EXPLORAR_RESERVA*.csv"), ("Todos os arquivos", "*.csv")],
        parent=root
    )
    ...
    # Gerar relatório
    relatorio = RelatorioReservasDetalhadas(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_HORAS_EXPLORAR_RESERVA {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
