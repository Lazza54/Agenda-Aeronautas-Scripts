# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Apresentação - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de apresentação divididas por períodos
"""
...
class RelatorioApresentacaoDetalhado:
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_apresentacao_detalhado.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE APRESENTACAO", titulo_style))
        ...
        total_diurno_geral = self.df['Tempo Apresentacao Diurno'].sum()
        total_noturno_geral = self.df['Tempo Apresentacao Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Apresentacao Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Apresentacao Especial Noturno'].sum()
        ...
        import time
        if len(anos) > 0:
            print("[DEBUG] Iniciando montagem do índice remissivo...")
            t0 = time.time()
            ...
            indice_data = [['ANO', 'DESCRICAO']]
            for ano in anos:
                if pd.isna(ano):
                    print(f"[DEBUG] Ano nulo ou inválido ignorado: {ano}")
                    continue
                try:
                    ano_int = int(ano)
                except Exception as e:
                    print(f"[ERRO] Não foi possível converter ano '{ano}' para inteiro: {e}")
                    continue
                # Garantir que ano_int é int e link_text é string
                ...
                try:
                    par_desc = Paragraph(f'Detalhamento de Apresentacao - {ano_int}', styles['Normal'])
                except Exception as e:
                    print(f"[ERRO] Falha ao criar Paragraph da descrição: {e}")
                    par_desc = Paragraph(f"Erro descrição ano {ano_int}", styles['Normal'])
                # Adiciona o link e a descrição à tabela de índice
                indice_data.append([par_link, par_desc])
            t1 = time.time()
            print(f"[DEBUG] Montagem do índice remissivo concluída em {t1-t0:.3f} segundos.")
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Apresentacao Diurno'].sum()
            total_noturno = df_ano['Tempo Apresentacao Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Apresentacao Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Apresentacao Especial Noturno'].sum()
            ...
            resumo_data = [
                ...
                ['SOMATORIA', 
                 self._formatar_timedelta(total_diurno + (total_especial_diurno * 2)),
                 self._formatar_timedelta((total_noturno * 2) + (total_especial_noturno * 3)),
                 self._formatar_timedelta(total_geral + total_especial),
                 self._formatar_timedelta(total_geral_calculado)]
            ]
            
            resumo_table = Table(resumo_data, colWidths=[4.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 3*cm])
            ...
            for idx, row in df_ano.iterrows():
                ...
                d = self._formatar_timedelta(row['Tempo Apresentacao Diurno'])
                n = self._formatar_timedelta(row['Tempo Apresentacao Noturno'])
                ed = self._formatar_timedelta(row['Tempo Apresentacao Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Apresentacao Especial Noturno'])
                
                total_linha = (row['Tempo Apresentacao Diurno'] + 
                              (row['Tempo Apresentacao Noturno'] * 2) +
                              (row['Tempo Apresentacao Especial Diurno'] * 2) + 
                              (row['Tempo Apresentacao Especial Noturno'] * 3))
            ...
            total_detalhamento = (total_diurno + (total_noturno * 2) + 
                                 (total_especial_diurno * 2) + (total_especial_noturno * 3))
            detalhes_data.append([
                'TOTAL', '',
                self._formatar_timedelta(total_diurno),
                self._formatar_timedelta(total_noturno),
        ...
        if len(anos) > 1:
            ...
            resumo_geral_data = [
                ...
                ['SOMATORIA',
                 self._formatar_timedelta(total_diurno_geral + (total_especial_diurno_geral * 2)),
                 self._formatar_timedelta((total_noturno_geral * 2) + (total_especial_noturno_geral * 3)),
                 self._formatar_timedelta(total_geral_geral + total_especial_geral),
                 self._formatar_timedelta(total_calc_geral)]
            ]
            
            resumo_geral_table = Table(resumo_geral_data, colWidths=[4.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 3*cm])
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE APRESENTACAO")
...
def main():
    print("="*80)
    print("GERADOR DE RELATORIO DETALHADO DE APRESENTACAO (OTIMIZADO)")
    ...
    if arquivo_csv_env and os.path.isfile(arquivo_csv_env):
        ...
        print(f"Arquivo selecionado (ENV): {os.path.basename(arquivo_csv)}")
    else:
        root = tk.Tk()
        root.withdraw()
        ...
        arquivo_csv = filedialog.askopenfilename(
            title="Selecione o arquivo CSV PROCESSADO de Apresentacao",
            filetypes=[("Arquivos CSV", "*APRESENTACAO*.csv"), ("Todos os arquivos", "*.csv")],
            parent=root
        )
    ...
    relatorio = RelatorioApresentacaoDetalhado(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_APRESENTACAO {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
    nome_saida = os.path.join(
        os.path.dirname(arquivo_csv),
        f"{nome_prefixo} {timestamp}.pdf"
    )
