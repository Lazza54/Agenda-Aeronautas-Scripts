# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Plantão - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de plantão divididas por períodos
"""
...
class RelatorioPlantaoDetalhado:
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_plantao_detalhado.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE PLANTAO", titulo_style))
        ...
        total_diurno_geral = self.df['Tempo Plantao Diurno'].sum()
        total_noturno_geral = self.df['Tempo Plantao Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Plantao Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Plantao Especial Noturno'].sum()
        ...
        if len(anos) > 0:
            ...
            for ano in anos:
                link_text = f'<link href="#ano_{int(ano)}" color="blue"><u>Ano {int(ano)}</u></link>'
                indice_data.append([
                    Paragraph(link_text, styles['Normal']),
                    Paragraph(f'Detalhamento de Plantao - {int(ano)}', styles['Normal'])
                ])
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Plantao Diurno'].sum()
            total_noturno = df_ano['Tempo Plantao Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Plantao Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Plantao Especial Noturno'].sum()
            ...
            detalhes_data = [['DATA', 'BASE', 'DIURNO', 'NOTURNO', 
                              'ESPECIAL\nDIURNO', 'ESPECIAL\nNOTURNO', 'TOTAL']]
            ...
            for idx, row in df_ano.iterrows():
                ...
                # Extrair base do plantão
                    base_plantao = ""
                    if 'Dep' in self.df.columns:
                        ...
                        if pd.isna(dep_valor) or str(dep_valor).lower() in ['nan', 'none', '']:
                            ...
                            base_plantao = str(dep_valor).strip()
                
                d = self._formatar_timedelta(row['Tempo Plantao Diurno'])
                n = self._formatar_timedelta(row['Tempo Plantao Noturno'])
                ed = self._formatar_timedelta(row['Tempo Plantao Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Plantao Especial Noturno'])
                
                total_linha = (row['Tempo Plantao Diurno'] + 
                              (row['Tempo Plantao Noturno'] * 2) +
                              (row['Tempo Plantao Especial Diurno'] * 2) + 
                              (row['Tempo Plantao Especial Noturno'] * 3))
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE PLANTAO")
...
def main():
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV PROCESSADO de Plantao (com tempos calculados)",
        filetypes=[("Arquivos CSV", "*PLANTAO*.csv"), ("Todos os arquivos", "*.csv")],
        parent=root
    )
    ...
    # Gerar relatório
    relatorio = RelatorioPlantaoDetalhado(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_HORAS_PLANTAO {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
