# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Treinamento - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de treinamento divididas por períodos
"""
...
class RelatorioTreinamentoDetalhado:
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_treinamento_detalhado.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE TREINAMENTO", titulo_style))
        ...
        total_diurno_geral = self.df['Tempo Treinamento Diurno'].sum()
        total_noturno_geral = self.df['Tempo Treinamento Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Treinamento Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Treinamento Especial Noturno'].sum()
        ...
        if len(anos) > 0:
            ...
            for ano in anos:
                ...
                indice_data.append([
                    ...
                    Paragraph(f'Detalhamento de Treinamento - {ano_int}', styles['Normal'])
                ])
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Treinamento Diurno'].sum()
            total_noturno = df_ano['Tempo Treinamento Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Treinamento Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Treinamento Especial Noturno'].sum()
            ...
            detalhes_data = [['DATA', 'TIPO TREINO', 'DIURNO', 'NOTURNO', 
                              'ESPECIAL\nDIURNO', 'ESPECIAL\nNOTURNO', 'TOTAL']]
            ...
            for idx, row in df_ano.iterrows():
                ...
                # Extrair tipo de treinamento (usar Activity ou criar campo específico)
                tipo_treino = ""
                if 'Activity' in self.df.columns:
                    tipo_treino = str(row.get('Activity', ''))
                
                d = self._formatar_timedelta(row['Tempo Treinamento Diurno'])
                n = self._formatar_timedelta(row['Tempo Treinamento Noturno'])
                ed = self._formatar_timedelta(row['Tempo Treinamento Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Treinamento Especial Noturno'])
                
                total_linha = (row['Tempo Treinamento Diurno'] + 
                              (row['Tempo Treinamento Noturno'] * 2) +
                              (row['Tempo Treinamento Especial Diurno'] * 2) + 
                              (row['Tempo Treinamento Especial Noturno'] * 3))
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE TREINAMENTO")
...
def main():
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV PROCESSADO de Treinamento (com tempos calculados)",
        filetypes=[("Arquivos CSV", "*TREINAMENTO*.csv"), ("Todos os arquivos", "*.csv")],
        parent=root
    )
    ...
    # Gerar relatório
    relatorio = RelatorioTreinamentoDetalhado(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_HORAS_TREINAMENTO {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
