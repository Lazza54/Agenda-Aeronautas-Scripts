...
def gerar_nome_csv_saida_base(nome_csv_entrada: str) -> str:
    """
    Gera o nome base do arquivo CSV de saída (sem timestamp), substituindo '_QUARTA_VERSAO' por '_APRESENTACAO'.
    """
    base_nome = os.path.basename(nome_csv_entrada)
    nome_sem_ext, _ = os.path.splitext(base_nome)

    # Tenta encontrar e substituir '_QUARTA_VERSAO'
    if '_QUARTA_VERSAO' in nome_sem_ext:
        # Pega a parte do nome antes de '_QUARTA_VERSAO'
        prefix = nome_sem_ext.split('_QUARTA_VERSAO')[0]
        return f"{prefix}_APRESENTACAO.csv" # Alterado para _APRESENTACAO.csv
    
    # Fallback para outros casos: se '_QUARTA_VERSAO' não for encontrado,
    # tenta remover o timestamp (se houver) e adicionar '_APRESENTACAO'
    timestamp_pattern = r'_\d{8}_\d{6}$' # Exemplo: _YYYYMMDD_HHMMSS
    cleaned_name = re.sub(timestamp_pattern, '', nome_sem_ext)
    
    # Se o nome já terminar com _APRESENTACAO, não adiciona novamente
    if not cleaned_name.endswith('_APRESENTACAO'):
        return f"{cleaned_name}_APRESENTACAO.csv"
    
    return f"{cleaned_name}.csv" # Caso já termine com _APRESENTACAO

def determinar_diretorio_e_arquivo():
    """
    Função para permitir ao usuário selecionar o arquivo CSV de entrada
    e o diretório para salvar o arquivo de saída.
    """
...
def parse_date_from_excel_serial(serial_value):
    ...
    return pd.to_datetime(serial_value, unit='D', origin='1899-12-30')
...
# Função adaptada para calcular Apresentacao e Pagamento
def calculate_apresentacao_and_payment_durations(checkin_dt: datetime, start_dt: datetime, holidays_set: set) -> tuple:
    """
    Calcula os tempos diurnos/noturnos (normais e especiais) para Apresentacao e Pagamento
    de um determinado intervalo (checkin_dt, start_dt).
    ...
    if pd.isna(checkin_dt) or pd.isna(start_dt) or checkin_dt >= start_dt:
        return (timedelta(0),) * 8

    # Duração total do evento (Checkin ao Start)
    total_event_duration = start_dt - checkin_dt

    # --- Cálculo de Tempos Diurno/Noturno Padrão ---
    # Apresentacao: Noturno 18:00 - 06:00
    apresentacao_night_total = get_interval_overlap_with_repeating_period(checkin_dt, start_dt, 18, 0, 6, 0)
    apresentacao_day_total = total_event_duration - apresentacao_night_total

    # Pagamento: Noturno 21:00 - 09:00
    pay_night_total = get_interval_overlap_with_repeating_period(checkin_dt, start_dt, 21, 0, 9, 0)
    pay_day_total = total_event_duration - pay_night_total

    # --- Cálculo de Tempos Diurno/Noturno Especiais ---
    apresentacao_special_day = timedelta(0)
    apresentacao_special_night = timedelta(0)
    ...
    while current_dt < start_dt:
        ...
        if eh_especial:
            # Apresentacao: verificar se é noturno (18:00-06:00)
            if current_hour >= 18 or current_hour < 6:
                apresentacao_special_night += delta
            else:
                apresentacao_special_day += delta
    ...
    return (apresentacao_day_total, apresentacao_night_total, apresentacao_special_day, apresentacao_special_night,
...
def processar_dados_aeronautica():
    ...
    try:
        ...
        # 'checkin' e 'start' são cruciais para esta nova tarefa
    ...
    # Condição 1: 'activity_prefix' deve estar em 'tipos_voo_data'
    activity_filter_condition = df['activity_prefix'].isin(tipos_voo_data) if tipos_voo_data else pd.Series(True, index=df.index)
    
    # Condição 2: 'id_leg' deve ser '-I' ou '-IF'
    # Garante que 'id_leg' não é nulo antes da comparação e converte para string para segurança
    id_leg_filter_condition = df['id_leg'].notna() & df['id_leg'].astype(str).isin(['-I', '-IF'])

    # Combina as duas condições
    if tipos_voo_data: # Aplica a filtragem se houver dados em tipos_voo.json
        df_processed = df[activity_filter_condition & id_leg_filter_condition].copy()
        print(f"Total de linhas lidas: {initial_row_count}. Linhas após filtrar por tipos_voo E Id_Leg: {len(df_processed)}")

        # Fallback para LATAM: em alguns arquivos o prefixo da atividade (ex.: LA3128 -> LA)
        # não existe em tipos_voo.json. Nesses casos, usa heurística de voo + Id_Leg.
        nome_arquivo_upper = os.path.basename(input_csv_path).upper()
        if len(df_processed) == 0 and "LATAM" in nome_arquivo_upper:
            print("Aviso: filtro por tipos_voo retornou 0 linhas para LATAM. Aplicando fallback por padrão de número de voo + Id_Leg.")
            atividade_voo_latam = (
                df['activity']
                .astype(str)
                .str.strip()
            ...
            df_processed = df[atividade_voo_latam & id_leg_filter_condition].copy()
        ...
        df_processed = df[id_leg_filter_condition].copy()
    ...
    # --- CALCULA 'Tempo Apresentacao' (TOTAL) ANTES DOS DETALHES ---
    # Certifica que checkin e start são válidos e checkin < start antes de calcular
    df_processed['tempo_apresentacao_total'] = pd.Timedelta(0) # Inicializa com zero
    valid_total_apresentacao_mask = df_processed['checkin'].notna() & df_processed['start'].notna() & (df_processed['checkin'] < df_processed['start'])
    df_processed.loc[valid_total_apresentacao_mask, 'tempo_apresentacao_total'] = \
        df_processed.loc[valid_total_apresentacao_mask, 'start'] - df_processed.loc[valid_total_apresentacao_mask, 'checkin']
    
    # --- 6. Adicionar e Calcular as 8 Novas Colunas (Apresentacao e Pagamento) ---
    new_internal_columns = [
        'tempo_apresentacao_diurno', 'tempo_apresentacao_noturno',
        'tempo_apresentacao_especial_diurno', 'tempo_apresentacao_especial_noturno',
        'pagamento_diurno', 'pagamento_noturno',
        'pagamento_especial_diurno', 'pagamento_especial_noturno'
    ]
    
    # Inicializa as novas colunas no df_processed com timedelta de 0
    for col in new_internal_columns:
        df_processed[col] = pd.Timedelta(0) 

    print("Calculando os novos tempos de apresentação e pagamento...")
    
    # Aplica a função de cálculo a cada linha
    # Verifica explicitamente se 'checkin' e 'start' são NaT e se 'checkin' é anterior a 'start'
    # Reutiliza a máscara de validação de datas para o cálculo detalhado
    valid_rows_mask = df_processed['checkin'].notna() & df_processed['start'].notna() & (df_processed['checkin'] < df_processed['start'])
    
    # Aplica a função apenas nas linhas válidas, usando 'checkin' e 'start'
    if valid_rows_mask.any():
        results = df_processed.loc[valid_rows_mask].apply(
            lambda row: calculate_apresentacao_and_payment_durations(row['checkin'], row['start'], feriados_data),
            axis=1,
            result_type='expand' 
        )

        # Atribui os resultados de volta ao DataFrame nas linhas válidas
        results.columns = new_internal_columns
        df_processed.loc[valid_rows_mask, new_internal_columns] = results
    else:
        print("Aviso: nenhuma linha válida para cálculo detalhado de apresentação/pagamento.")

    # Garante que todas as novas colunas são do tipo Timedelta e preenche NaT com 0
    for col in new_internal_columns:
    ...
    final_output_columns_pascal_case = [
        ...
        'Tempo Apresentacao', # Nova coluna adicionada aqui
        'Tempo Apresentacao Diurno', 'Tempo Apresentacao Noturno',
        'Tempo Apresentacao Especial Diurno', 'Tempo Apresentacao Especial Noturno',
    ...
    internal_to_output_name_map = {
        ...
        'checkout': 'Checkout',
        'tempo_apresentacao_total': 'Tempo Apresentacao', # Mapeamento para a nova coluna total
        'tempo_apresentacao_diurno': 'Tempo Apresentacao Diurno',
        'tempo_apresentacao_noturno': 'Tempo Apresentacao Noturno',
        'tempo_apresentacao_especial_diurno': 'Tempo Apresentacao Especial Diurno',
        'tempo_apresentacao_especial_noturno': 'Tempo Apresentacao Especial Noturno',
        'pagamento_diurno': 'Pagamento Diurno',
        'pagamento_noturno': 'Pagamento Noturno',
        'pagamento_especial_diurno': 'Pagamento Especial Diurno',
        'pagamento_especial_noturno': 'Pagamento Especial Noturno'
    }
    ...
    for internal_col, output_col in internal_to_output_name_map.items():
        ...
        # Se a coluna interna não existir, ela já está como NaN no df_output vazio
    ...
    # Formatar colunas Timedelta para string 'days HH:MM:SS' ou 'HH:MM:SS'
    timedelta_cols_for_format = [
        'Tempo Apresentacao', # Adicionada aqui para formatação
        'Tempo Apresentacao Diurno', 'Tempo Apresentacao Noturno',
        'Tempo Apresentacao Especial Diurno', 'Tempo Apresentacao Especial Noturno',
        'Pagamento Diurno', 'Pagamento Noturno',
        'Pagamento Especial Diurno', 'Pagamento Especial Noturno'
    ...
    base_output_filename = gerar_nome_csv_saida_base(input_csv_path) # Gera nome base como "LAZZARINI_VCP__APRESENTACAO.csv"
    timestamp = datetime.now().strftime("%d%m%Y_%H%M%S") # Gera timestamp como "27102023_103045"
