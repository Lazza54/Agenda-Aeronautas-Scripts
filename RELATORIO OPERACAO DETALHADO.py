# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Operação - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de operação divididas por períodos
"""
...
class RelatorioOperacaoDetalhado:
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            # Verificar se colunas necessárias existem (nomes como vem do CSV processado)
            colunas_necessarias = [
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_operacao_detalhado.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE OPERACAO", titulo_style))
        ...
        total_diurno_geral = self.df['Tempo Operacao Diurno'].sum()
        total_noturno_geral = self.df['Tempo Operacao Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Operacao Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Operacao Especial Noturno'].sum()
        ...
        if len(anos) > 0:
            ...
            for ano in anos:
                ...
                indice_data.append([
                    ...
                    Paragraph(f'Detalhamento de Operacao - {ano_int}', styles['Normal'])
                ])
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Operacao Diurno'].sum()
            total_noturno = df_ano['Tempo Operacao Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Operacao Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Operacao Especial Noturno'].sum()
            ...
            for idx, row in df_ano.iterrows():
                ...
                d = self._formatar_timedelta(row['Tempo Operacao Diurno'])
                n = self._formatar_timedelta(row['Tempo Operacao Noturno'])
                ed = self._formatar_timedelta(row['Tempo Operacao Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Operacao Especial Noturno'])
                
                total_linha = (row['Tempo Operacao Diurno'] + 
                              (row['Tempo Operacao Noturno'] * 2) +
                              (row['Tempo Operacao Especial Diurno'] * 2) + 
                              (row['Tempo Operacao Especial Noturno'] * 3))
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE OPERACAO")
...
def main():
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV PROCESSADO de Operacao (com tempos calculados)",
        filetypes=[("Arquivos CSV", "*OPERACAO*.csv"), ("Todos os arquivos", "*.csv")],
        parent=root
    )
    ...
    # Gerar relatório
    relatorio = RelatorioOperacaoDetalhado(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_HORAS_OPERACAO {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
