...
"""
Gerador de Relatório de Tempo de Operação
...
class RelatorioOperacao:
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            colunas_tempo = [
                'Tempo Operacao',
                'Tempo Operacao Diurno',
                'Tempo Operacao Noturno', 
                'Tempo Operacao Especial Diurno',
                'Tempo Operacao Especial Noturno'
            ]
        ...
        for coluna in colunas_tempo:
            ...
            for registro in registros:
                if coluna in registro and not pd.isna(registro[coluna]):
                    ...
                    if coluna != 'Tempo Operacao':  # Evitar contar duplicado
                        total_geral += registro[coluna]
            sumario[coluna] = total
        
        # Se não há total calculado, usar o campo Tempo Operacao
        if total_geral == pd.Timedelta(0):
            for registro in registros:
                if 'Tempo Operacao' in registro and not pd.isna(registro['Tempo Operacao']):
                    total_geral += registro['Tempo Operacao']
        
        sumario['Tempo Operacao Total'] = total_geral
        return sumario
    ...
    def criar_tabela_registros(self, registros, styles):
        ...
        for registro in registros:
            ...
            # Calcular total do tempo de operação
            tempo_total = pd.Timedelta(0)
            if 'Tempo Operacao' in registro and not pd.isna(registro['Tempo Operacao']):
                tempo_total = registro['Tempo Operacao']
            else:
                # Fallback: somar as categorias
                for col in ['Tempo Operacao Diurno', 'Tempo Operacao Noturno', 
        ...
        # Criação da tabela
        # 8 colunas: Atividade (2.2cm), Apresentação (2.6cm), Partida (2.6cm), De (1.3cm), Para (1.3cm), Fim (2.6cm), Chegada (2.6cm), Tempo Operação Total (2.8cm) = 18cm
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas de Operação - TOTAL', self.formatar_tempo(sumario.get('Tempo Operacao Total', pd.Timedelta(0)))],
            ['Horas de Operação Diurnas', self.formatar_tempo(sumario.get('Tempo Operacao Diurno', pd.Timedelta(0)))],
            ['Horas de Operação Noturnas', self.formatar_tempo(sumario.get('Tempo Operacao Noturno', pd.Timedelta(0)))],
            ['Horas de Operação Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Operacao Especial Diurno', pd.Timedelta(0)))],
            ['Horas de Operação Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Operacao Especial Noturno', pd.Timedelta(0)))]
        ]
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_operacao.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE HORAS DE OPERAÇÃO", titulo_style))
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE HORAS DE OPERAÇÃO")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Operação...")
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV de Operação",
        filetypes=[
            ("Arquivos CSV", "*.csv"),
            ("Todos os arquivos", "*.*")
        ]
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioOperacao(arquivo_csv)
    ...
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_OPERACAO_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
