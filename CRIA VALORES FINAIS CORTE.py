...
def gerar_nome_csv_saida_base(nome_csv_entrada: str) -> str:
    """
    Gera o nome base do arquivo CSV de saída (sem timestamp), substituindo '_QUARTA_VERSAO'
    e sufixos de etapas anteriores por '_TEMPO_CORTE'.
    """
    ...
    previous_stage_suffixes = ['_APRESENTACAO', '_OPERACAO', '_QUARTA_VERSAO']
    ...
    if not cleaned_name.endswith('_TEMPO_CORTE'):
        return f"{cleaned_name}_TEMPO_CORTE.csv"
    
    return f"{cleaned_name}.csv" # Caso já termine com _TEMPO_CORTE
...
def calculate_main_activity_and_payment_durations(activity_start_dt: datetime, activity_end_dt: datetime, holidays_set: set) -> tuple:
    ...
    # Duração total da atividade principal (ex: Tempo Corte, Apresentacao)
    ...
    # Atividade Principal (Tempo Corte/Apresentacao): Noturno 18:00 - 06:00
...
def processar_dados_aeronautica():
    ...
    try:
        ...
        # 'end' e 'checkout' são cruciais para esta nova tarefa de Tempo Corte
    ...
    # Condição 2: 'id_leg' deve ser '-F' ou '-IF'
    # Garante que 'id_leg' não é nulo antes da comparação e converte para string para segurança
    id_leg_filter_values_for_tempo_corte = ['-F', '-IF'] # Valores específicos para Tempo Corte
    id_leg_filter_condition = df['id_leg'].notna() & df['id_leg'].astype(str).isin(id_leg_filter_values_for_tempo_corte)
    ...
    if tipos_voo_data: # Aplica a filtragem se houver dados em tipos_voo.json
        ...
        # Fallback para LATAM: ex. LA3128 -> prefixo "LA" não está em tipos_voo.json.
    ...
    # --- CALCULA 'Tempo Corte' (TOTAL) ANTES DOS DETALHES ---
    # Certifica que 'end' e 'checkout' são válidos e 'end' < 'checkout' antes de calcular
    df_processed['tempo_corte_total'] = pd.Timedelta(0) # Inicializa com zero
    valid_total_tempo_corte_mask = df_processed['end'].notna() & df_processed['checkout'].notna() & (df_processed['end'] < df_processed['checkout'])
    df_processed.loc[valid_total_tempo_corte_mask, 'tempo_corte_total'] = \
        df_processed.loc[valid_total_tempo_corte_mask, 'checkout'] - df_processed.loc[valid_total_tempo_corte_mask, 'end']
    
    # --- 6. Adicionar e Calcular as 8 Novas Colunas (Tempo Corte e Pagamento) ---
    # Os nomes internos representam a saída da função `calculate_main_activity_and_payment_durations`
    ...
    print("Calculando os novos tempos de corte e pagamento...")
    
    # Aplica a função de cálculo a cada linha, usando 'end' e 'checkout'
    # Reutiliza a máscara de validação de datas para o cálculo detalhado
    valid_rows_for_calc_mask = df_processed['end'].notna() & df_processed['checkout'].notna() & (df_processed['end'] < df_processed['checkout'])
    
    if valid_rows_for_calc_mask.any():
        results = df_processed.loc[valid_rows_for_calc_mask].apply(
            lambda row: calculate_main_activity_and_payment_durations(row['end'], row['checkout'], feriados_data),
            axis=1,
            result_type='expand' 
        )
        ...
        df_processed.loc[valid_rows_for_calc_mask, new_internal_calculated_columns] = results
    else:
        print("Aviso: nenhuma linha válida para cálculo detalhado de corte/pagamento.")
    ...
    final_output_columns_pascal_case = [
        ...
        'Tempo Corte', # Nova coluna total de Tempo Corte
        'Tempo Corte Diurno', 'Tempo Corte Noturno',
        'Tempo Corte Especial Diurno', 'Tempo Corte Especial Noturno',
    ...
    internal_to_output_name_map = {
        ...
        'checkout': 'Checkout',
        'tempo_corte_total': 'Tempo Corte', # Mapeamento para a nova coluna total de Tempo Corte
        'main_activity_day_total': 'Tempo Corte Diurno', # Mapeia de generic para Tempo Corte
        'main_activity_night_total': 'Tempo Corte Noturno', # Mapeia de generic para Tempo Corte
        'main_activity_special_day': 'Tempo Corte Especial Diurno', # Mapeia de generic para Tempo Corte
        'main_activity_special_night': 'Tempo Corte Especial Noturno', # Mapeia de generic para Tempo Corte
