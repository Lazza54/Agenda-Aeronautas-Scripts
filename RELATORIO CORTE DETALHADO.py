# -*- coding: utf-8 -*-
"""
Relatório Detalhado de Horas de Tempo Corte - Diurnas, Noturnas e Especiais
Gera PDF com análise detalhada das horas de tempo corte divididas por períodos
"""
...
class RelatorioCorteDetalhado:
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            colunas_necessarias = [
                'Tempo Corte Diurno',
                'Tempo Corte Noturno',
                'Tempo Corte Especial Diurno',
                'Tempo Corte Especial Noturno'
            ]
    ...
    # ... (restante do código segue igual ao que você anexou, mas com os totais corrigidos e nomeação do PDF ajustada)

    
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_corte_detalhado.pdf"):
        ...
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE TEMPO CORTE", titulo_style))
        ...
        # Totais gerais
        total_diurno_geral = self.df['Tempo Corte Diurno'].sum()
        total_noturno_geral = self.df['Tempo Corte Noturno'].sum()
        total_especial_diurno_geral = self.df['Tempo Corte Especial Diurno'].sum()
        total_especial_noturno_geral = self.df['Tempo Corte Especial Noturno'].sum()
        ...
        if len(anos) > 0:
            ...
            for ano in anos:
                ...
                indice_data.append([
                    ...
                    Paragraph(f'Detalhamento de Tempo Corte - {ano_int}', styles['Normal'])
                ])
        ...
        for idx_ano, ano in enumerate(anos):
            ...
            total_diurno = df_ano['Tempo Corte Diurno'].sum()
            total_noturno = df_ano['Tempo Corte Noturno'].sum()
            total_especial_diurno = df_ano['Tempo Corte Especial Diurno'].sum()
            total_especial_noturno = df_ano['Tempo Corte Especial Noturno'].sum()
            ...
            for idx, row in df_ano.iterrows():
                ...
                if 'Dep' in self.df.columns and 'Arr' in self.df.columns:
                    ...
                    # Tratar apenas valores realmente vazios (NAT é válido)
                ...
                d = self._formatar_timedelta(row['Tempo Corte Diurno'])
                n = self._formatar_timedelta(row['Tempo Corte Noturno'])
                ed = self._formatar_timedelta(row['Tempo Corte Especial Diurno'])
                en = self._formatar_timedelta(row['Tempo Corte Especial Noturno'])
                
                total_linha = (row['Tempo Corte Diurno'] + 
                              (row['Tempo Corte Noturno'] * 2) +
                              (row['Tempo Corte Especial Diurno'] * 2) + 
                              (row['Tempo Corte Especial Noturno'] * 3))
    ...
    def _gerar_relatorio_vazio(self, nome_arquivo):
        ...
        try:
            ...
            c.drawCentredString(width/2, height - 100, "EXTRATO DEMONSTRATIVO DETALHADO DE HORAS DE TEMPO CORTE")
...
def main():
    """Funcao principal otimizada"""
    print("="*80)
    print("GERADOR DE RELATORIO DETALHADO DE TEMPO CORTE (OTIMIZADO)")
    print("Diurnas, Noturnas e Especiais")
    print("="*80)
    print()
    print("IMPORTANTE: Selecione o arquivo CSV PROCESSADO que foi gerado pelo")
    print("script 'CRIA VALORES FINAIS CORTE.py'")
    ...
    if arquivo_csv_env and os.path.isfile(arquivo_csv_env):
        ...
        arquivo_csv = filedialog.askopenfilename(
            title="Selecione o arquivo CSV PROCESSADO de Tempo Corte (com tempos calculados)",
            filetypes=[("Arquivos CSV", "*TEMPO_CORTE*.csv"), ("Todos os arquivos", "*.csv")],
            parent=root
        )
    ...
    relatorio = RelatorioCorteDetalhado(arquivo_csv)
    nome_prefixo = f"{relatorio.nome_aeronauta} {relatorio.re} SUMARIO_TEMPO_CORTE {relatorio.empresa} {relatorio.cargo} {relatorio.periodo}".strip()
