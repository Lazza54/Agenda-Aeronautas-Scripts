...
"""
Gerador de Relatório de Jornada
...
class RelatorioJornada:
        ...
        for coluna in colunas_tempo:
            ...
            for registro in registros:
                if coluna in registro and not pd.isna(registro[coluna]):
                    ...
                    if coluna != 'Tempo Jornada':  # Evitar contar duplicado
                        total_geral += registro[coluna]
            sumario[coluna] = total
        
        # Se não há total calculado, usar o campo Tempo Jornada
        if total_geral == pd.Timedelta(0):
            for registro in registros:
                if 'Tempo Jornada' in registro and not pd.isna(registro['Tempo Jornada']):
                    total_geral += registro['Tempo Jornada']
        
        sumario['Tempo Jornada Total'] = total_geral
        return sumario
    ...
    def criar_tabela_registros(self, registros, styles):
        ...
        for registro in registros:
            ...
            # Calcular total do tempo de jornada
            tempo_total = pd.Timedelta(0)
            if 'Tempo Jornada' in registro and not pd.isna(registro['Tempo Jornada']):
                tempo_total = registro['Tempo Jornada']
            else:
                # Fallback: somar as categorias
                for col in ['Tempo Jornada Diurno', 'Tempo Jornada Noturno', 
        ...
        # Criação da tabela
        # Ajuste de larguras para evitar encavalamento e remover coluna extra:
        tabela = Table(
            dados_tabela,
            colWidths=[2.2*cm, 3.2*cm, 3.2*cm, 1.5*cm, 1.5*cm, 3.2*cm, 3.2*cm, 2.2*cm]
        )
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas de Jornada - TOTAL', self.formatar_tempo(sumario.get('Tempo Jornada Total', pd.Timedelta(0)))],
            ['Horas de Jornada Diurnas', self.formatar_tempo(sumario.get('Tempo Jornada Diurno', pd.Timedelta(0)))],
            ['Horas de Jornada Noturnas', self.formatar_tempo(sumario.get('Tempo Jornada Noturno', pd.Timedelta(0)))],
            ['Horas de Jornada Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Jornada Especial Diurno', pd.Timedelta(0)))],
            ['Horas de Jornada Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Jornada Especial Noturno', pd.Timedelta(0)))]
        ]
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_jornada.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE HORAS DE JORNADA", titulo_style))
        ...
        for periodo_semanal in sorted(self.dados_processados['semanal'].keys()):
            ...
            if registros:
                # Determinar domingo de início e sábado de término da semana
                primeira_data = min(r['Checkin'] for r in registros)
                ultima_data = max(r['Checkin'] for r in registros)
                # Domingo: weekday() == 6 (domingo) se isoweekday()==7, mas weekday()==6 é sábado, weekday()==0 é segunda
                # Python: weekday() == 6 é domingo? Não, weekday()==6 é sábado, weekday()==0 é segunda
                # Para domingo: subtrair weekday() dias
                domingo_inicio = primeira_data - timedelta(days=primeira_data.weekday() + 1) if primeira_data.weekday() != 6 else primeira_data
                # Para sábado: adicionar (5 - weekday()) dias se domingo==6, sábado==5
                sabado_fim = domingo_inicio + timedelta(days=6)
                titulo_periodo = f"Semana de {domingo_inicio.strftime('%d/%m/%Y')} a {sabado_fim.strftime('%d/%m/%Y')}"
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE JORNADA")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Jornada...")
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV de Jornada",
        filetypes=[
            ("Arquivos CSV", "*.csv"),
            ("Todos os arquivos", "*.*")
        ]
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioJornada(arquivo_csv)
    
    # Define nome do arquivo de saída
    diretorio = os.path.dirname(arquivo_csv)
    nome_base = os.path.splitext(os.path.basename(arquivo_csv))[0]
    nome_base = nome_base.replace("_TEMPO_JORNADA_", "_JORNADA_")
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_JORNADA_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
