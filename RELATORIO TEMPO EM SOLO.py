...
"""
Gerador de Relatório de Tempo em Solo
...
class RelatorioTempoEmSolo:
    ...
    def _extrair_dados_aeronauta(self):
        ...
        try:
            ...
            # Regra solicitada:
            # - Aeronauta: 3º e 4º campos do nome do arquivo
            # - Base: 5º campo
            # - RE: 6º campo
            # Considera campos não vazios separados por '_'.
            partes = [p.strip() for p in nome_sem_extensao.split('_') if p and p.strip()]
            ...
            if len(partes) >= 4:
                self.nome_aeronauta = f"{partes[2]} {partes[3]}".upper()
            if len(partes) >= 5:
                self.base = partes[4].upper()
            if len(partes) >= 6:
                self.re = partes[5]
    ...
    def agrupar_dados(self):
        ...
        for idx, row in self.df.iterrows():
            ...
            self.dados_processados['semanal'][ano_semana].append(row_dict)
            # Agrupamento mensal
            ano_mes = data_checkin.strftime('%Y-%m')
            self.dados_processados['mensal'][ano_mes].append(row_dict)
            # Agrupamento anual
            ano = data_checkin.strftime('%Y')
            self.dados_processados['anual'][ano].append(row_dict)
        ...
        for coluna in colunas_tempo:
            ...
            for registro in registros:
                if coluna in registro and not pd.isna(registro[coluna]):
                    ...
                    if coluna != 'Tempo Solo':  # Evitar contar duplicado
                        total_geral += registro[coluna]
            sumario[coluna] = total
        
        # Se não há total calculado, usar o campo Tempo Solo
        if total_geral == pd.Timedelta(0):
            for registro in registros:
                if 'Tempo Solo' in registro and not pd.isna(registro['Tempo Solo']):
                    total_geral += registro['Tempo Solo']
        
        sumario['Tempo Solo Total'] = total_geral
        return sumario
    
    def criar_tabela_registros(self, registros, styles):
        """Cria tabela com registros individuais"""
        # Cabeçalho da tabela (igual ao RELATORIO REPOUSO)
        cabecalho = [
        ...
        for registro in registros:
            ...
            if 'Tempo Solo' in registro and not pd.isna(registro['Tempo Solo']):
                tempo_total = registro['Tempo Solo']
            else:
                for col in ['Tempo Solo Diurno', 'Tempo Solo Noturno',
            ...
            # Filtra registros com tempo_total nulo ou zero
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas em Solo - TOTAL', self.formatar_tempo(sumario.get('Tempo Solo Total', pd.Timedelta(0)))],
            ['Horas em Solo Diurnas', self.formatar_tempo(sumario.get('Tempo Solo Diurno', pd.Timedelta(0)))],
            ['Horas em Solo Noturnas', self.formatar_tempo(sumario.get('Tempo Solo Noturno', pd.Timedelta(0)))],
            ['Horas em Solo Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Solo Especial Diurno', pd.Timedelta(0)))],
            ['Horas em Solo Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Solo Especial Noturno', pd.Timedelta(0)))]
        ]
    ...
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_tempo_em_solo.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE TEMPO EM SOLO", titulo_style))
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE TEMPO EM SOLO")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Tempo em Solo...")
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioTempoEmSolo(arquivo_csv)
    ...
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_TEMPO_EM_SOLO_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
