...
"""
Gerador de Relatório de Tempos de Repouso
...
class RelatorioRepouso:
        ...
        for coluna in colunas_tempo:
            ...
            for registro in registros:
                if coluna in registro and not pd.isna(registro[coluna]):
                    ...
                    if coluna != 'Tempo Repouso':  # Evitar contar duplicado
                        total_geral += registro[coluna]
            sumario[coluna] = total
        # Se não há total calculado, usar o campo Tempo Repouso
        if total_geral == pd.Timedelta(0):
            for registro in registros:
                if 'Tempo Repouso' in registro and not pd.isna(registro['Tempo Repouso']):
                    total_geral += registro['Tempo Repouso']
        sumario['Tempo Repouso Total'] = total_geral
        return sumario
    ...
    def criar_tabela_sumario(self, sumario, titulo_periodo):
        ...
        dados_sumario = [
            ...
            ['Horas de Repouso - TOTAL', self.formatar_tempo(sumario.get('Tempo Repouso Total', pd.Timedelta(0)))],
            ['Horas de Repouso Diurnas', self.formatar_tempo(sumario.get('Tempo Repouso Diurno', pd.Timedelta(0)))],
            ['Horas de Repouso Noturnas', self.formatar_tempo(sumario.get('Tempo Repouso Noturno', pd.Timedelta(0)))],
            ['Horas de Repouso Especiais Diurnas', self.formatar_tempo(sumario.get('Tempo Repouso Especial Diurno', pd.Timedelta(0)))],
            ['Horas de Repouso Especiais Noturnas', self.formatar_tempo(sumario.get('Tempo Repouso Especial Noturno', pd.Timedelta(0)))],
        ]
    ...
    def criar_tabela_registros(self, registros, styles):
        ...
        for registro in registros:
            ...
            if 'Tempo Repouso' in registro and not pd.isna(registro['Tempo Repouso']):
                tempo_total = registro['Tempo Repouso']
            else:
                for col in ['Tempo Repouso Diurno', 'Tempo Repouso Noturno', 
        ...
        return tabela
    def gerar_relatorio_pdf(self, nome_arquivo="relatorio_repouso.pdf"):
        ...
        # Título principal
        story.append(Paragraph("EXTRATO DEMONSTRATIVO DE HORAS DE REPOUSO", titulo_style))
        ...
        for periodo_semanal in sorted(self.dados_processados['semanal'].keys()):
            ...
            if registros:
                # Determinar período da semana (sempre domingo a sábado do calendário)
                qualquer_data = min(r['Checkin'] for r in registros)
                # Calcular domingo (início da semana): weekday() 0=seg..6=dom
        ...
        for periodo_mensal in sorted(self.dados_processados['mensal'].keys()):
            ...
            if registros:
                ...
                # Tabela de registros: exibir todas as linhas, independente da quantidade
                tabela_registros = self.criar_tabela_registros(registros, styles)
                story.append(tabela_registros)
                story.append(Spacer(1, 15))
...
def main():
    """Função principal"""
    print("="*60)
    print("🎯 GERADOR DE RELATÓRIO DE TEMPOS DE REPOUSO")
    print("="*60)
    ...
    print("\n📂 Selecione o arquivo CSV com os dados de Tempos de Repouso...")
    ...
    arquivo_csv = arquivo_csv_env if (arquivo_csv_env and os.path.isfile(arquivo_csv_env)) else filedialog.askopenfilename(
        title="Selecione o arquivo CSV de Tempos de Repouso",
        filetypes=[
            ("Arquivos CSV", "*.csv"),
            ("Todos os arquivos", "*.*")
        ]
    ...
    print("\n🔄 Processando dados...")
    gerador = RelatorioRepouso(arquivo_csv)
    ...
    nome_arquivo = os.path.join(
        diretorio,
        f"{nome_base}_RELATORIO_REPOUSO_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
